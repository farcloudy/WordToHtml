...
| `toggleBold()` / `setColor(hex\|null)` | 给选中的文字加粗／改色（`null` 恢复默认色） |
...
- 结构性操作（回车分段、退格合并、工具栏加粗改色、加批注）直接改模型，再重排渲染 ——
  因此界面上改的和导出 docx 用的永远是同一份模型，不会出现「界面改了、导出没改」。
...
| `npm run verify:p3` | 编辑操作纯函数 48 项单测 + 真实浏览器实测：**敲字后片段仍是同一个 DOM 节点**（证明正常输入没有重排）、插入符落在刚敲完的字后面、行数变了要重排时插入符按坐标找回、回车分段／退格合并、加粗改色、修订模式的增删标记、加批注（模型／侧栏／正文锚点三者对得上）、撤销重做、以及编辑后每页仍不溢出 |
