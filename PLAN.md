...
- **文件模板** = 样式、页边距与文档网格绑成一体（`DOC_TEMPLATES`）。`manager`「管理人文件」四边 25mm、
  样式走 `DEFAULT_SPEC`、**无文档网格**（`gridType: 'none'`，段前/段后因此只写磅值）；
  `govDoc`「简易公文格式」上 37 / 下 35 / 左 28 / 右 26 mm、整套样式覆盖在 `GOV_STYLES`
  （正文与各级标题三号 16pt、标题二号 22pt 方正小标宋简体、列表与页脚四号 14pt、
  行距一律 `lineRule: 'grid'` = 单倍行距吸附网格行）、网格是**指定行和字符网格**（每页 22 行、
  **Word 侧每行 28 字、预览每行 27 字** —— 原因见第 5 节，这是有意的偏差）；
  `MARGIN_PRESETS` 由 `DOC_TEMPLATES` 派生。
...
这一套**没有文档网格**（`gridType: 'none'`，Word 的默认排版）：导出的 docx 里 `w:docGrid` 不带 type，
段前／段后也只写磅值、不写 `w:beforeLines`（原因见下面「网格」那条与 `docx/lineUnits.ts`）。

**简易公文格式**（`DOC_TEMPLATES[1].spec.styles`，即 `GOV_STYLES`；字号取自 issues/20260916.md）：

| 类型 | 字体（中文／西文） | 字号 | 加粗 | 对齐 | 首行缩进 | 行距 | 段前／段后 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 文本标题 | 方正小标宋简体／Times New Roman | 22pt（二号） | — | 居中 | — | 单倍（吸附网格） | 0／1 行 |
| 一级标题 | 黑体／Times New Roman | 16pt（三号） | — | 两端 | 2 字符 | 单倍（吸附网格） | 0／0 |
| 二级标题 | 楷体／Times New Roman | 16pt | — | 两端 | 2 字符 | 单倍（吸附网格） | 0／0 |
| 三级标题 | 仿宋／Times New Roman | 16pt | — | 两端 | 2 字符 | 单倍（吸附网格） | 0／0 |
| 正文 | 仿宋／Times New Roman | 16pt | — | 两端 | 2 字符 | 单倍（吸附网格） | 0／0 |
| 抬头 | 仿宋／Times New Roman | 16pt | — | 左 | — | 单倍（吸附网格） | 0／0 |
| 落款 | 仿宋／Times New Roman | 16pt | — | 右 | — | 单倍（吸附网格） | 0／0 |
| 附件 | 黑体／黑体 | 16pt | — | 两端 | — | 单倍（吸附网格） | 0／1 行 |
...
- **行网格：每页 22 行**（`GOV_LINE_PT` = `page.gridLinePt` = 579 缇）：上 37 / 下 35 的版心高
  637.795pt ÷ 22 = 28.99pt，**取 28.95 才装得下 22 行**（22 × 28.95 = 636.90pt），取 29pt 只剩 21 行。
  八条样式的行距是 `lineRule: 'grid'`（**单倍行距吸附文档网格**，2026-09-19 用户要求）：Word 那侧写
  `w:lineRule="auto"`，一行正好吸附成一个 579 缇的网格行；预览那侧没有网格吸附，`resolveSpec()` 把
  这一档的 `linePt` 归一化成本套的网格行高 —— 两边仍然是「一行 = 一个网格行」。这条等价关系由
  `verify:pages` 拿预览页数与 Word 页数逐套模板对账（现在是 4/4 与 5/5）。
- **字网格：Word 每行 28 字、预览每行 27 字**。`PageSpec.gridCharsPerLine = 28` 让导出侧写
  `<w:docGrid w:type="linesAndChars" w:charSpace="…">`（Word 报 `LayoutMode=1`、CharsLine=28、
  LinesPage=22），字符间距由 Word 压出来；浏览器既不吸附网格、也不压缩字符间距，156mm 的版心
  只放得下 27 个三号字，所以**预览排 27 字** —— 这个偏差是有意的（2026-09-16 用户选择保标准
  156×225mm 版心）。`w:charSpace` 的算式是实测反推的：
  `charSpace = 204.8 × (版心宽 ÷ 每行字数 − 默认字号)`（都是缇；本机 Word 16.0 上 CharsLine 取
  20/24/26/27/28/29/30 七组数据，反推出的七个数与 Word 自己写的一字不差），细节与「为什么要留
  2 缇余量」见 `docx/export.ts` 的 `charSpaceOf`。顺带一条实测：这些中文字体的汉字宽度都是整 1em
  （仿宋/黑体/楷体/方正小标宋都一样），所以「每行几个字」只取决于版心宽 ÷ 字号，与选了哪个中文字体无关。
